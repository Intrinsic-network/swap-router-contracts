...
import { IUniswapV2Pair, IWRBTC, MockTimeSwapRouter02, TestERC20 } from '../typechain'
...
describe('SwapRouter', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    const { wrbtc, factory, factoryV2, router, tokens, nft } = await completeFixture(wallets, provider)
  ...
  function encodeUnwrapWRBTC(amount: number) {
    const functionSignature = 'unwrapWRBTC(uint256,address)'
    return solidityPack(
      ['bytes4', 'bytes'],
      [
  ...
  beforeEach('load fixture', async () => {
    ;({ router, wrbtc, factory, factoryV2, tokens, nft } = await loadFixture(swapRouterFixture))
  ...
  describe('swaps - v3', () => {
    async function createPoolWRBTC(tokenAddress: string) {
      ...
      return createV3Pool(wrbtc.address, tokenAddress)
    }
    ...
    describe('*WithFee', () => {
      ...
      it('#unwrapWRBTCWithFee', async () => {
        ...
        const functionSignature = 'unwrapWRBTCWithFee(uint256,address,uint256,address)'
  ...
  describe('swaps - v2', () => {
    ...
    let wrbtcPairs: IUniswapV2Pair[]

    async function createPoolWRBTC(token: TestERC20) {
      await wrbtc.deposit({ value: liquidity })
      return createV2Pool((wrbtc as unknown) as TestERC20, token)
    }
    ...
    describe('#swapExactTokensForTokens', () => {
      async function exactInput(
        ...
        const params: [number, number, string[], string] = [
          ...
          amountOutMinimum,
          tokens,
          outputIsWRBTC ? ADDRESS_THIS : MSG_SENDER,
        ]
      ...
      describe('RBTC input', () => {
        describe('WRBTC', () => {
          beforeEach(async () => {
            const pair = await createPoolWRBTC(tokens[0])
            wrbtcPairs = [pair]
          })

          it('WRBTC -> 0', async () => {
            // get balances before
            const poolBefore = await getBalances(wrbtcPairs[0].address)
            const traderBefore = await getBalances(trader.address)
            await expect(exactInput([wrbtc.address, tokens[0].address]))
              .to.emit(wrbtc, 'Deposit')
              .withArgs(router.address, 2)
            // get balances after
            const poolAfter = await getBalances(wrbtcPairs[0].address)
            const traderAfter = await getBalances(trader.address)
            expect(traderAfter.token0).to.be.eq(traderBefore.token0.add(1))
            expect(poolAfter.wrbtc).to.be.eq(poolBefore.wrbtc.add(2))
            expect(poolAfter.token0).to.be.eq(poolBefore.token0.sub(1))
          })

          it('WRBTC -> 0 -> 1', async () => {
            const traderBefore = await getBalances(trader.address)
            await expect(exactInput([wrbtc.address, tokens[0].address, tokens[1].address], 3))
              .to.emit(wrbtc, 'Deposit')
              .withArgs(router.address, 3)
            const traderAfter = await getBalances(trader.address)
            expect(traderAfter.token1).to.be.eq(traderBefore.token1.add(1))
      ...
      describe('RBTC output', () => {
        describe('WRBTC', () => {
          beforeEach(async () => {
            const pair0 = await createPoolWRBTC(tokens[0])
            const pair1 = await createPoolWRBTC(tokens[1])
            wrbtcPairs = [pair0, pair1]
          })
          ...
          it('0 -> 1 -> WRBTC', async () => {
            ...
            await expect(exactInput([tokens[0].address, tokens[1].address, wrbtc.address], 3))
              .to.emit(wrbtc, 'Withdrawal')
              .withArgs(router.address, 1)
    ...
    describe('#swapTokensForExactTokens', () => {
      ...
      describe('RBTC input', () => {
        describe('WRBTC', () => {
          ...
          it('WRBTC -> 0 -> 1', async () => {
            const traderBefore = await getBalances(trader.address)
            await expect(exactOutput([wrbtc.address, tokens[0].address, tokens[1].address], 1, 3))
              .to.emit(wrbtc, 'Deposit')
              .withArgs(router.address, 3)
            const traderAfter = await getBalances(trader.address)
            expect(traderAfter.token1).to.be.eq(traderBefore.token1.add(1))
      ...
      describe('RBTC output', () => {
        describe('WRBTC', () => {
          ...
          it('0 -> 1 -> WRBTC', async () => {
            // get balances before
            const traderBefore = await getBalances(trader.address)
            await expect(exactOutput([tokens[0].address, tokens[1].address, wrbtc.address], 1, 3))
              .to.emit(wrbtc, 'Withdrawal')
              .withArgs(router.address, 1)
            // get balances after
            const traderAfter = await getBalances(trader.address)
